...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=kareemelkasaby/multi-server:$SHA
kubectl set image deployments/client-deployment client=kareemelkasaby/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=kareemelkasaby/multi-worker:$SHA